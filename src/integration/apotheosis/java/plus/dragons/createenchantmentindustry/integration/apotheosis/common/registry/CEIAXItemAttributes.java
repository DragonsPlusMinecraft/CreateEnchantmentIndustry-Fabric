...
public class CEIAXItemAttributes {
    public static final Supplier<ItemAttributeType> CAN_BE_SALVAGED = singleton("can_be_salvaged",
            "can be Salvaged",
            "cannot be Salvaged",
            SalvagingHelper::canSalvage);
    public static final Supplier<ItemAttributeType> HAS_GEM = singleton("has_gem",
            "has gem socketed",
            "has no socketed gem",
            (stack, level) -> SocketHelper.getGems(stack).stream().anyMatch(GemInstance::isValid));

    public static final Supplier<ItemAttributeType> HAS_EMPTY_SOCKET = singleton("has_empty_socket",
            "has empty socket",
            "has no empty socket",
            (stack, level) -> SocketHelper.hasEmptySockets(stack));

    public static final Supplier<ItemAttributeType> IS_UPGRADABLE_GEM = singleton("is_upgradable_gem",
            "is upgradable gem",
            "is not upgradable gem",
            (stack, level) -> GemCutterBlockEntity.isUpgradableGem(stack));

    public static final Supplier<ItemAttributeType> HAS_UPGRADABLE_AFFIX = singleton("has_upgradable_affix",
            "has upgradable affix",
            "has no upgradable affix",
            (stack, level) -> AffixAugmentorBlockEntity.hasUpgradableAffix(stack));

    public static final Supplier<ItemAttributeType> GEM_PURITY = complex("gem_purity",
            "is a gem of %1$s purity",
            "is not a gem of %1$s purity",
            GemPurityAttributes.Type::new);

    private static Supplier<ItemAttributeType> singleton(
        ...
        return register(name, new SingletonItemAttribute.Type(type -> new SingletonItemAttribute(
                type, predicate, CEIACommon.ID + "." + name)));
    }

    private static Supplier<ItemAttributeType> complex(
            String name, String description, String invertedDescription, Supplier<ItemAttributeType> supplier) {
        String descriptionKey = "create.item_attributes." + CEIACommon.ID + "." + name;
        String invertedDescriptionKey = descriptionKey + ".inverted";
        REGISTRATE.addRawLang(descriptionKey, description);
        REGISTRATE.addRawLang(invertedDescriptionKey, invertedDescription);
    ...
    private static Supplier<ItemAttributeType> register(String name, ItemAttributeType type) {
        ItemAttributeType registered = Registry.register(
