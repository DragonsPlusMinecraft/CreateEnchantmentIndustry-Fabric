...
public class BlazeComposerScene {
    private static final ResourceLocation DEMO_AFFIX = new ResourceLocation("zenith", "sword/attribute/violent");
    private static final ResourceLocation DEMO_RARITY = new ResourceLocation("zenith", "rare");
    ...
    public static void superComposing(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        ItemStack pearl = new ItemStack(Adventure.Items.MYTHIC_MATERIAL);
    ...
    public static void automate(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(20);
        scene.world().modifyBlockEntity(composerPos, BlazeComposerBlockEntity.class,
                be -> insertAutomationItem(be, demo.affixedEquipment()));
        scene.idle(15);
        instructInputArm(scene, inputArmPos, templateDepotPos, demo.blankTemplate(), 1);
        scene.idle(20);
        scene.world().modifyBlockEntity(composerPos, BlazeComposerBlockEntity.class,
                be -> insertAutomationItem(be, demo.blankTemplate()));
        ...
        scene.overlay().showText(80)
                .text("The output Arm extracts the filled template and stripped equipment one after the other")
                .attachKeyFrame();
        ...
        instructOutputArm(scene, outputArmPos, composerPos, demo.filledTemplate());
        scene.idle(20);
        ...
        instructOutputArm(scene, outputArmPos, composerPos, demo.strippedEquipment());
        scene.idle(20);
        scene.world().modifyBlockEntity(outputDepotPos, DepotBlockEntity.class,
    ...
    private static void instructOutputArm(
        ...
        scene.idle(20);
        scene.world().modifyBlockEntity(composerPos, BlazeComposerBlockEntity.class,
                BlazeComposerScene::extractAutomationItem);
        scene.world().instructArm(armPos, ArmBlockEntity.Phase.SEARCH_OUTPUTS, stack, -1);
        scene.idle(20);
        scene.world().instructArm(armPos, ArmBlockEntity.Phase.MOVE_TO_OUTPUT, stack, 0);
    }
    ...
    private static void fillNormalTank(BlazeComposerBlockEntity composer) {
        composer.getNormalTank().setFluid(CEIFluidUnits.stackUnits(
                CEIAXFluids.APOTHEOTIC_ESSENCE.getSource(), composer.getNormalTank().getCapacity()));
    }

    private static void fillSupplyTank(FluidTankBlockEntity tank, int amount) {
        var controller = tank.getControllerBE();
        if (controller != null)
            CEITransfer.insertMillibuckets(
                    controller.getTankInventory(), CEIAXFluids.APOTHEOTIC_ESSENCE.getSource(), amount, false);
    }
    ...
    private static void transferEssence(
        ...
        scene.world().modifyBlockEntity(supplyTankPos, FluidTankBlockEntity.class, tank -> {
            var controller = tank.getControllerBE();
            if (controller != null)
                CEITransfer.extractMillibuckets(
                        controller.getTankInventory(), CEIAXFluids.APOTHEOTIC_ESSENCE.getSource(), amount, false);
        });
        scene.world().modifyBlockEntity(composerPos, BlazeComposerBlockEntity.class, composer -> {
            var handler = composer.getFluidStorage(Direction.DOWN);
            if (handler != null)
                CEITransfer.insertMillibuckets(
                        handler, CEIAXFluids.APOTHEOTIC_ESSENCE.getSource(), amount, false);
        });
    }
    ...
    private static void addDemoAffix(ItemStack stack, float level) {
        AffixHelper.setRarity(stack, RarityRegistry.INSTANCE.holder(DEMO_RARITY).get());
        OverlimitAffixHelper.setAffixLevel(stack, AffixRegistry.INSTANCE.holder(DEMO_AFFIX), level);
    }

    private static void insertAutomationItem(BlazeComposerBlockEntity composer, ItemStack stack) {
        try (Transaction transaction = Transaction.openOuter()) {
            composer.insertAutomationItem(stack.copy(), transaction);
            transaction.commit();
        }
    }

    private static void extractAutomationItem(BlazeComposerBlockEntity composer) {
        try (Transaction transaction = Transaction.openOuter()) {
            composer.extractAutomationItem(1, transaction);
            transaction.commit();
        }
    }
